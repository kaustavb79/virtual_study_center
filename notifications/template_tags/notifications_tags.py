...
# @register.simple_tag(takes_context=True)
# def live_notification_badge(context, badge_class='live_notify_badge'):
#     user = user_context(context)
#     nf_type_unread = Notification.objects.filter(recipient=user).unread()
#     counter = 0
#     for item in nf_type_unread:
#         data = item.data
#         nf_type = data.nf_type
#         if (nf_type=='Notification'):
#             counter+=1
...
#     html = "<span class='{badge_class}'>{unread}</span>".format(
#         badge_class=badge_class, unread=counter
#     )
#     return format_html(html)

@register.simple_tag(takes_context=True)
def live_notification_badge(context, badge_class='live_notification_badge'):
    user = user_context(context)
    counter = Notification.objects.filter(recipient=user, notification_type='Notification').unread().count()
    if not user:
        return ''
    ...
    print('notifications--', counter)
    return format_html(html)

@register.simple_tag(takes_context=True)
def live_message_notification_badge(context, badge_class='live_message_notification_badge'):
    user = user_context(context)
    counter = Notification.objects.filter(recipient=user, notification_type='Message').unread().count()
    if not user:
        return ''
...
@register.simple_tag(takes_context=True)
def live_calendar_notification_badge(context, badge_class='live_calendar_notification_badge'):
    user = user_context(context)
    counter = 0
    if not user:
        return ''
