...
//	displayAlert("Please allow microphone and camera access to your current browser", "error", 100000)
    ...
    var confirmation_obj = {
        "icon":"",
        "head":"Camera and microphone are blocked",
        "body":body,
        "width":"400px",
        "action_type":"alert",
        "action_yes":action_yes,
...
//        $(".mic-input").val(device_label);
         // fix refresh issue
        set_stream_by_id(deviceId, selected_device_callback_fn_if)
...
//                set_selected_audio_device(reset_mic);
    ...
    // Call every time a user changes his device
         // fix refresh issue
        set_stream_by_id(deviceId, selected_device_callback_fn_else)
...
function set_stream_by_id(myPreferredDeviceId, selected_device_callback){
//    console.warn('--- set_stream_by_id ---')
    var constraints = {}
    var is_video_stream = false;
    var is_audio_stream = false;

    var is_id_valid = check_device_id_exist(myPreferredDeviceId);
...
debugger;
    ...
    if(
    (!jQuery.isEmptyObject( currentMic ))&&
    (!jQuery.isEmptyObject( currentCam ))
    ){
        force_stop_all_stream()
        dcc_close()
        hide_connectivity_test()
    }else{
        displayAlert("Please select microphone and camera device","error",3000)
    }
}


function force_stop_all_stream(){
...
function check_device_id_exist(device_id){
    var is_valid = false
    var device_found = {};
